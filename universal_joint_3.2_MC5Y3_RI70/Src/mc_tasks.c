...
__weak uint8_t TSK_HighFrequencyTask(void)
{
  ...
  /* USER CODE BEGIN HighFrequencyTask 1 */
	g_previous_electrical_position = g_current_electrical_position; // store old position
	g_current_electrical_position = ENCODER_M1._Super.hElAngle;

	if (g_previous_electrical_position > INT16_MAX / 2 && g_current_electrical_position < INT16_MIN / 2)
	{
		g_current_electrical_rotation++;
	}
	if (g_current_electrical_position > INT16_MAX / 2  && g_previous_electrical_position < INT16_MIN / 2)
	{
		g_current_electrical_rotation--;
	}
