...
//  NTC_Clear( &g_TempBearingSensorParamsM1 );
...
//					// szukam sektora
////					if (abs(g_joint_status.mc_current_electric_position) < INT16_MAX / 5)
////					{
//						g_current_sector_number = get_rotation_number_from_calibration_table(0, g_joint_configuration.number_of_sectors - 1, g_ma730.angle, g_joint_configuration.calibration_table_1[0], 1);
////					}
//
...
//					{
//						// ustalenie aktualnego obrotu elektrycznego w orientacji absolutnej
//						if (g_current_sector_number - g_previous_sector_number > 0) // obrot w lewo -> CCW
//						{ // rotation in CCW direction => "+"
//							g_current_electrical_rotation = (g_current_sector_number) * g_joint_configuration.calibration_sector_size;
//						}
//						else // obrot w prawo -> CW
//						{ // rotation in CW direction ==> "-"
//							g_current_electrical_rotation = (g_current_sector_number + 1) * g_joint_configuration.calibration_sector_size - 1;
//						}
////							if (g_current_electrical_rotation != g_joint_status.mc_current_electric_rotation)
////							{
////								g_joint_status.mc_current_electric_rotation = g_current_electrical_rotation; // ustawienie oborotu elektrycznego wzgledem kalibracji
////
////							}
//						g_joint_status.mc_current_electric_rotation = g_current_electrical_rotation; // ustawienie oborotu elektrycznego wzgledem kalibracji
//						g_joint_status.mc_current_electric_position = g_current_electrical_position;
//
//						// ustalenie polozenia aktualnego obrotu elektrycznego w orientacji absolutnej
//						if (g_joint_status.mc_current_electric_rotation > g_joint_configuration.zero_electric_rotation) // obrot w kierunku dodatnim
//						{
//							g_diff_zero_electric_position 		= ((float) ((int32_t) INT16_MAX - (int32_t) g_joint_configuration.zero_electric_position)) / UINT16_MAX; // od prawego brzegu
//							g_diff_current_electric_position 	= ((float) ((int32_t) INT16_MAX + (int32_t) g_joint_status.mc_current_electric_position))  / UINT16_MAX; // od lewego brzegu
//							g_electric_offset_to_zero_in_rad   	= (abs(g_joint_status.mc_current_electric_rotation - g_joint_configuration.zero_electric_rotation) + g_diff_zero_electric_position + g_diff_current_electric_position) * g_joint_configuration.electric_rotation_width;
//						}
//						else if (g_joint_status.mc_current_electric_rotation < g_joint_configuration.zero_electric_rotation) // obrot w kierunku ujemnym
//						{
//							g_diff_zero_electric_position 		= ((float) ((int32_t) INT16_MAX + (int32_t) g_joint_configuration.zero_electric_position)) / UINT16_MAX; // od prawego brzegu
//							g_diff_current_electric_position 	= ((float) ((int32_t) INT16_MAX - (int32_t) g_joint_status.mc_current_electric_position))  / UINT16_MAX; // od lewego brzegu
//							g_electric_offset_to_zero_in_rad   	= (-1 * abs(g_joint_status.mc_current_electric_rotation - g_joint_configuration.zero_electric_rotation) - g_diff_zero_electric_position - g_diff_current_electric_position) * g_joint_configuration.electric_rotation_width;
//						}
//						else // ten sam obrot elektryczny - przypadek moze wystapic gdy obrot "0" jest na brzegu sektora
//						{
//							if (g_joint_status.mc_current_electric_position < g_joint_configuration.zero_electric_position) { // jestesmy po prawej stronie "0"
//
//							}
//							else // jestesmy po lewej stronie "0"
//							{
//
//							}
//
...
////						if (g_joint_status.mc_current_electric_rotation > g_joint_configuration.zero_electric_rotation) // obrot w kierunku dodatnim
////						{
////							g_diff_zero_electric_position 		= ((float) ((int32_t) INT16_MAX - (int32_t) g_joint_configuration.zero_electric_position)) / UINT16_MAX;
////							g_diff_current_electric_position 	= ((float) ((int32_t) INT16_MAX - (int32_t) g_joint_status.mc_current_electric_position)) / UINT16_MAX;
////							g_electric_offset_to_zero_in_rad   	= (g_joint_status.mc_current_electric_rotation - g_joint_configuration.zero_electric_rotation + g_diff_zero_electric_position + g_diff_current_electric_position) * g_joint_configuration.electric_rotation_width;
////						}
////						else if (g_joint_status.mc_current_electric_rotation < g_joint_configuration.zero_electric_rotation) // obrot w kierunku ujemnym
////						{
////							g_diff_zero_electric_position = ((float) ((int32_t) INT16_MAX - (int32_t) g_joint_configuration.zero_electric_position)) / UINT16_MAX;
////							g_diff_current_electric_position = ((float) ((int32_t) INT16_MAX - (int32_t) g_joint_status.mc_current_electric_position)) / UINT16_MAX;
////							g_electric_offset_to_zero_in_rad   = (g_joint_status.mc_current_electric_rotation - g_joint_configuration.zero_electric_rotation + g_diff_zero_electric_position + g_diff_current_electric_position) * g_joint_configuration.electric_rotation_width;
//////							l_diff_electric_position = (UINT16_MAX - (INT16_MAX + g_joint_status.mc_current_electric_position)) + (INT16_MAX + g_joint_configuration.zero_electric_position);
////						}
////						else // ten sam obrot elektryczny
////						{
////
////						}
//						// wyliczenia pozycji
////						int32_t l_diff_electric_position = g_joint_configuration.zero_electric_position - g_joint_status.mc_current_electric_position;
////						int32_t l_diff_electric_position = ((int32_t) g_joint_configuration.zero_electric_position + INT16_MAX) - ((int32_t) g_joint_status.mc_current_electric_position + INT16_MAX);
////						l_electric_offset_to_zero_in_rad   = -1 * ((g_joint_configuration.zero_electric_rotation - g_joint_status.mc_current_electric_rotation) + ((float) l_diff_electric_position / UINT16_MAX) - 1.0) * electric_rotation_width;
////						float l_electric_offset_to_zero_in_rad   = -1 * ((g_joint_configuration.zero_electric_rotation - g_joint_status.mc_current_electric_rotation) - 1.0) * electric_rotation_width;
//
//						g_encoder_offset_to_current_in_rad = ((float) g_joint_status.mc_current_motor_position_multiturn / (ENCODER_M1.PulseNumber / g_joint_configuration.pole_pairs)) * g_joint_configuration.electric_rotation_width;
//
//						g_temp_1 = g_diff_zero_electric_position;
//						g_temp_2 = abs(g_joint_status.mc_current_electric_rotation - g_joint_configuration.zero_electric_rotation);
//						g_temp_3 = g_diff_current_electric_position;
//						g_temp_4 = g_encoder_offset_to_current_in_rad;
//						g_temp_5 = g_joint_status.mc_current_motor_position;
//						g_temp_6 = ENCODER_M1.PreviousCapture;
//						g_temp_7 = g_ma730.angle;
//						//						g_temp_4 = g_encoder_offset_to_current_in_rad;
//
////						g_temp_1 = g_diff_zero_electric_position;
////						g_temp_2 = g_diff_current_electric_position;
////						g_temp_3 = abs(g_joint_status.mc_current_electric_rotation - g_joint_configuration.zero_electric_rotation);
////						g_temp_4 = g_encoder_offset_to_current_in_rad;
//
//						//							g_temp_1 = g_electric_offset_to_zero_in_rad;
////							g_temp_2 = g_encoder_offset_to_current_in_rad;
//
...
//						g_joint_status.f_current_encoder_position_offset = g_electric_offset_to_zero_in_rad - g_encoder_offset_to_current_in_rad;
////						g_joint_status.f_current_encoder_position_offset = g_electric_offset_to_zero_in_rad;
//
////							g_temp_5 = g_current_electrical_rotation;
////							g_temp_6 = g_current_electrical_position;
//
////						if (g_current_electrical_rotation == 994)
////						{
////							g_temp_6 = g_current_electrical_position;
////						}
//
...
//				int16_t l_speed_in_dpp = g_joint_status.f_current_joint_speed * (float) INT16_MAX / M_TWOPI;
			...
			{
				int32_t l_temp_1 = (int32_t) (g_temp_1 * INT32_MAX);
				int32_t l_temp_3 = (int32_t) (g_temp_3 * INT32_MAX);
				int32_t l_temp_4 = (int32_t) (g_temp_4 * INT32_MAX);
